public class AboutUs {

    public static void showAboutUs() {
        System.out.println("=========== About Us ===========");
        System.out.println("Welcome to I3 Typing Master v2026");
        System.out.println();
        System.out.println("This application is designed to help users improve");
        System.out.println("their typing speed and accuracy through practice.");
        System.out.println();
        System.out.println("Features:");
        System.out.println("- User Login and Registration");
        System.out.println("- Typing Test Practice");
        System.out.println("- Performance Tracking");
        System.out.println();
        System.out.println("Developed by: ITC Students");
        System.out.println("Year: 2026");
        System.out.println("================================");
    }
} 
